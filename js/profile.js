...
editBtns.forEach(function (editBtn) {
    editBtn.addEventListener('click', function (event) {
        ...
        // Get the closest parent element with the class 'row'
        let row = event.target.closest('.row');

        // Get the value of the 'data-input' attribute
        let inputName = row.getAttribute('data-input');

        // Get all input elements within the row
        let inputFields = row.querySelectorAll('.edit-form input');

        // Toggle the display of each input element
...
let confirmBtns = document.querySelectorAll('#confirmation-btn')

confirmBtns.forEach(function(confirmBtn){
    
    confirmBtn.addEventListener('click', function(event){
        event.preventDefault();
        let confirmationPasswordInput = document.getElementById("confirmation-password");
        document.querySelector('.edit-confirmation').style.display="block";
        confirmationPasswordInput.scrollIntoView({behavior: "smooth"});
        setTimeout(function() {
            confirmationPasswordInput.focus();
            confirmationPasswordInput.style.border= "solid 1px var(--color-secondary)";
            confirmationPasswordInput.style.background= "#fff";
        }, 500);
        
    });
});

labels = document.getElementsByI('label');
